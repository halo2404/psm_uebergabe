...
#$functions_modus{"PSM_REGRESSION"}=[\&getPsmRegression,\&createPutFile,\&getCdrs,\&doReplace,\&createGenerate,\&Generate,\&doTar,\&doPut];
...
#$functions_modus{"PSM_REGRESSION"}=[\&getPsmRegression,\&createPutFile,\&getCdrs,\&doReplace,\&createGenerate,\&doTar,\&doPut,\&doErase];  # <- aktuelle Version
#$functions_modus{"PSM_REGRESSION"}=[\&getPsmRegression,\&createPutFile,\&getCdrs,\&doReplace1,\&createGenerate,\&doTar,\&doPut];  # <- zu Testzwecken
...
if ( $modus eq "" )
  {
    ...
    printf "Mögliche Werte für <MODUS> sind :\n\n" ;
...
sub current_packages($verzeichnis, $pattern, $pattern1)
{
  ...
  foreach ( @current_packages )
  {
    ...
    open (FILE,"$result") or die "Konnte Datei ${base_ccb20b}/${verzeichnis}/${input} nicht öffnen: $!\n";
    open (OUT, "+<$result");
    while (my $line=<FILE>){   
...
foreach ( @current_packages )
{
  ...
  open (FILE,"$result") or die "Konnte Datei ${base_ccb20b}/AMA.src/${input} nicht öffnen: $!\n";
  open (OUT, "+<$result");
  while (my $line=<FILE>){   
         $line=~s/$pattern/$pattern1/g;
         printf OUT $line;
...
sub Error()
{
  open (FILE,">fehler.txt") or die "Konnte Datei fehler.txt nicht öffnen: $!\n";
  while (my $line=<FILE>){        
         printf FILE $line;
  }
